...
from costa.training.model_restore import load_model_and_checkpoint_files
# from nnunet.training.network_training.nnUNetTrainer import nnUNetTrainer
# # if we use custom nnUNetTrainer, we should use the following trainer
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
                  ...
                  segmentation_export_kwargs: dict = None, disable_postprocessing: bool = False, img_path_root=None):
    """
    ...
    all_output_files = []
    for preprocessed in track(preprocessing, description="Predicting...", total=len(list_of_lists)):
        ...
        tmp = trainer.predict_preprocessed_data_return_seg_and_softmax(
        ...
        results.append(pool.starmap_async(save_segmentation_nifti_from_softmax,
                                          ((softmax, output_filename, dct, interpolation_order, region_class_order,
                                            None, None,
                                            npz_file, None, force_separate_z, interpolation_order_z, True, logits, img_path_root),)
                                          ))
...
def predict_cases_original(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
                  num_threads_nifti_save, segs_from_prev_stage=None, do_tta=True, mixed_precision=True,
                  overwrite_existing=False,
                  all_in_gpu=False, step_size=0.5, checkpoint_name="model_final_checkpoint",
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
    ...
    if mode == "normal":
        ...
        return predict_cases(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
                             ...
                             disable_postprocessing=disable_postprocessing, img_path_root=img_path_root)
    elif mode == "fast":
        if overwrite_all_in_gpu is None:
            all_in_gpu = False
        else:
