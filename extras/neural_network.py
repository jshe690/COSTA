...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
                   ...
                   verbose: bool = True, mixed_precision: bool = True) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
    ...
    def predict_3D_original(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
                   use_sliding_window: bool = False,
                   step_size: float = 0.5, patch_size: Tuple[int, ...] = None, regions_class_order: Tuple[int, ...] = None,
                   use_gaussian: bool = False, pad_border_mode: str = "constant",
                   pad_kwargs: dict = None, all_in_gpu: bool = False,
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if all_in_gpu:
            ...
            logits = torch.zeros([self.num_classes] + list(data.shape[1:]), dtype=torch.half,
                                             device=self.get_device())
            ...
            logits = np.zeros([self.num_classes] + list(data.shape[1:]), dtype=np.float32)
        ...
        for x in steps[0]:
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    ...
                    tmp = self._internal_maybe_mirror_and_pred_3D(
                        data[None, :, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z], mirror_axes, do_mirroring,
                        gaussian_importance_map)
                    predicted_patch = tmp[0][0]
                    logits_patch = tmp[1][0]

                    if all_in_gpu:
                        predicted_patch = predicted_patch.half()
                        logits_patch = logits_patch.half()
                    else:
                        predicted_patch = predicted_patch.cpu().numpy()
                        logits_patch = logits_patch.cpu().numpy()

                    aggregated_results[:, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z] += predicted_patch
                    logits[:, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z] += logits_patch
        ...
        logits /= aggregated_nb_of_predictions
        ...
        return predicted_segmentation, aggregated_results, logits

    def _internal_predict_3D_3Dconv_tiled_original(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
                                          patch_size: tuple, regions_class_order: tuple, use_gaussian: bool,
                                          pad_border_mode: str, pad_kwargs: dict, all_in_gpu: bool,
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        result_torch = torch.zeros([1, self.num_classes] + list(x.shape[2:]),
                                   dtype=torch.float)
        logits = torch.zeros([1, self.num_classes] + list(x.shape[2:]),
                                   dtype=torch.float)
        ...
        for m in range(mirror_idx):
            ...
            if m == 1 and (2 in mirror_axes):
                print('WARNING: LOGITS NOT BEING MIRRORED!!!!!!!')
                print('WARNING: LOGITS NOT BEING MIRRORED!!!!!!!')
                print('WARNING: LOGITS NOT BEING MIRRORED!!!!!!!')
                print('WARNING: LOGITS NOT BEING MIRRORED!!!!!!!')
                print('WARNING: LOGITS NOT BEING MIRRORED!!!!!!!')
        ...
        return result_torch, logits

    def _internal_maybe_mirror_and_pred_3D_original(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
                                           do_mirroring: bool = True,
