...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
                                         ...
                                         interpolation_order_z: int = 0, verbose: bool = True, logits=None, img_path_root=None):
    """
    ...
    if np.any([i != j for i, j in zip(np.array(current_shape[1:]), np.array(shape_original_after_cropping))]):
        ...
        logits_old_spacing = resample_data_or_seg(logits, shape_original_after_cropping, is_seg=False,
                                               axis=lowres_axis, order=order, do_separate_z=do_separate_z,
                                               order_z=interpolation_order_z)
    ...
    if bbox is not None:
        ...
        logits_old_size = np.zeros(shape_original_before_cropping, dtype=np.float32)
        logits_old_size = np.expand_dims(logits_old_size, axis=0)
        logits_old_size = np.repeat(logits_old_size, 2, axis=0)

        for c in range(3):
            bbox[c][1] = np.min((bbox[c][0] + seg_old_spacing.shape[c], shape_original_before_cropping[c]))

        logits_old_size[0][bbox[0][0]:bbox[0][1],
        bbox[1][0]:bbox[1][1],
        bbox[2][0]:bbox[2][1]] = logits_old_spacing[0]
        logits_old_size[1][bbox[0][0]:bbox[0][1],
        bbox[1][0]:bbox[1][1],
        bbox[2][0]:bbox[2][1]] = logits_old_spacing[1]
    else:
    ...
    print('logits_old_size shape', logits_old_size.shape)
    tag = out_fname.split('/')[-1].split('_0000')[0] # if using raw img files
    # tag = out_fname.split('/')[-1].split('_0000')[0].join('_0000') # if using nnunet formatted ("_0000" suffixed) files
    # img_path_root = f'/eresearch/brain/jshe690/personal/Jiantao/IXI16/images'
    img_path = f'{img_path_root}/{tag}'
    logits_save_path = out_fname[:-7] + f"_logits.nii.gz"
    print('tag', tag)
    print('out_fname', out_fname)
    print('img_path', img_path)
    print('logits_save_path', logits_save_path)

    logits_transposed = np.transpose(logits_old_size, [3, 2, 1, 0])
    img_raw = nib.load(img_path)
    new_image = nib.Nifti1Image(logits_transposed, affine=img_raw.affine, header=img_raw.header)
    nib.save(new_image, logits_save_path)
    # Save LOGITS #######################################################
...
def save_segmentation_nifti_from_softmax_original(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
                                         properties_dict: dict, order: int = 1,
                                         region_class_order: Tuple[Tuple[int]] = None,
                                         seg_postprogess_fn: callable = None, seg_postprocess_args: tuple = None,
                                         resampled_npz_fname: str = None,
